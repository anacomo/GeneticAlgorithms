...
def selection_pair(population: Population, fitness_function: FitnessFunction) -> Population:
    ...
    """
    first_element = choices(
        population = population,
        weights = [fitness_function(chromosome) for chromosome in population],
        k = 1
    )
    population.remove(first_element[0])
    second_element = choices(
        population = population,
        weights = [fitness_function(chromosome) for chromosome in population],
        k = 1
    )
    population.remove(second_element[0])
...
def mutation(chromosome: Chromosome, mutation_probability: float) -> Chromosome:
    ...
    chromosome[index] = chromosome[index] if random() > mutation_probability else abs(chromosome[index] - 1)
    return chromosome

def selection_probability(chromosome: Chromosome, population: Population, parameters: Parameters, bounds: Bounds, precision: int) -> float:
...
def show_function(x_axis: List[float], y_axis: List[float], bounds: Bounds, parameters: Parameters, precision: int):
    ...
    # actual best result
    # x2 = array([i for i in x1 if apply_function(parameters, i) == max(y)])
    # y2 = array([apply_function(parameters, i) for i in x if apply_function(parameters, i) == max(y)])
    # file.write("The optimal solution is " + str(x2[0]) + "\n\n")

    colors = [y1[0] / i * (10 ** precision) for i in y1]
...
def show_crossover_probability(population: Population, parameters: Parameters, bounds: Bounds, precision: int):
    for i in population:
        file.write("X: " + str(chromosome_to_float(i, precision)) + "  Crossover probability: " + str(selection_probability(i, population, parameters, bounds, precision)) + "\n")
...
def evolution( 
    ...
    crossover_population = int(crossover_probability * len(population))
    #file.write(str(crossover_population))

    for i in range(generation_limit):
        population = sorted(population, key = lambda chromosome: fitness_function(chromosome), reverse = True)
        next_generation = population[0 : 2] # elitistic selection
        population = population[2 : ]
        for j in range(crossover_population // 2):
            parents = selection_function(population, fitness_function)
            file.write("Chose parents: " + str(chromosome_to_float(parents[0])) + " and " + str(chromosome_to_float(parents[1])) + "\n")
            #print(chromosome_to_float(parents[0]), chromosome_to_float(parents[1]))
            offspring_a, offspring_b = crossover_function(parents[0], parents[1])
            offspring_a, offspring_b = mutation_function(offspring_a), mutation_function(offspring_b)
            next_generation += [offspring_a, offspring_b]

        next_generation += population
        population = next_generation
        file.write(str([chromosome_to_float(i) for i in population]))
...
def main():
    ...
    final_population = evolution(
        ...
        mutation_function = partial(mutation, mutation_probability = mutation_probability),
